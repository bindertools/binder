// Native backend for the Version Control app. Thin C-ABI wrapper around the
// existing git_ops::dispatch (cpp/src/git.cpp) -- LoadLibrary'd by the host
...
BINDER_APP_EXPORT int binder_app_dispatch(const char* type, const char* args_json_utf8,
    ...
    *out_json = nullptr;
    ...
    json resp;
    if (!git_ops::dispatch(type, msg, req_id ? req_id : "", resp)) return 0;
